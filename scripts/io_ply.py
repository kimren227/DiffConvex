...
def read_ply(filename):
    ...
    if fmt == 'ascii':
        ...
        with open(filename, 'rb') as ply:
            ...
            data["vertices"] = torch.tensor(np.stack((points_np["x"],points_np["y"],points_np["z"]), axis=1).astype(np.float32), device='cuda')
            if "nx" in points_np.dtype.fields.keys():
                data["normals"] = torch.tensor(np.stack((points_np["nx"],points_np["ny"],points_np["nz"]), axis=1).astype(np.float32), device='cuda')
            if mesh_size:
                ...
                data["faces"] = torch.tensor(np.stack((mesh_np["v1"], mesh_np["v2"], mesh_np["v3"]), axis=1).astype(np.int32), device='cuda')
