...
def compute_vertex_normals(verts, faces, face_normals):
    ...
    # print(torch.norm(normals, dim=0))
    # print((normals / torch.norm(normals, dim=0)).transpose(0, 1).shape)
    # print((normals / torch.norm(normals, dim=0)).transpose(0, 1).norm(dim=-1))

    return (normals / (torch.norm(normals, dim=0)+1e-6)).transpose(0, 1)
